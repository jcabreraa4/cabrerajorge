...
function MessageHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex max-w-full min-w-0 items-center px-4 text-xs font-medium tracking-wide text-muted-foreground uppercase group-has-data-[variant=ghost]/message:px-0', className)}
      {...props}
    />
  );
}
...
function MessageFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('flex max-w-full min-w-0 items-center px-4 text-xs font-medium tracking-wide text-muted-foreground uppercase group-has-data-[variant=ghost]/message:px-0 group-data-[align=end]/message:justify-end', className)}
      {...props}
    />
  );
}
