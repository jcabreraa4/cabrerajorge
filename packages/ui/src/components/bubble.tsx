...
function BubbleContent({ className, render, ...props }: useRender.ComponentProps<'div'>) {
  return useRender({
    ...
    props: mergeProps<'div'>(
      {
        className: cn('w-fit max-w-full min-w-0 overflow-hidden rounded-none border border-transparent px-2.5 py-2 text-xs leading-relaxed wrap-break-word group-data-[align=end]/bubble:self-end [button]:text-left [button,a]:transition-colors [button,a]:outline-none [button,a]:focus-visible:border-ring [button,a]:focus-visible:ring-1 [button,a]:focus-visible:ring-ring/50', className)
      },
      props
    ),
    render,
...
const bubbleReactionsVariants = cva('absolute z-10 flex w-fit shrink-0 items-center justify-center gap-1 rounded-none bg-muted px-1.5 py-0.5 text-xs ring-2 ring-card has-[button]:p-0', {
  variants: {
    side: {
