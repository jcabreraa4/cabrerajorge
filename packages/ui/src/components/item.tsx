...
const itemVariants = cva('group/item flex w-full flex-wrap items-center rounded-none border text-sm transition-colors duration-100 outline-none focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 [a]:transition-colors [a]:hover:bg-muted', {
  variants: {
    variant: {
...
function ItemTitle({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn('line-clamp-1 flex w-fit items-center gap-2 text-xs leading-snug font-semibold uppercase underline-offset-4', className)}
      {...props}
    />
  );
}
...
function ItemDescription({ className, ...props }: React.ComponentProps<'p'>) {
  return (
    <p
      ...
      className={cn('line-clamp-2 text-left text-sm leading-relaxed font-normal text-muted-foreground [&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-primary', className)}
      {...props}
    />
  );
}
